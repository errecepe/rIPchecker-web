	...
	<header>
		<nav>
			<table><tr><th>Grupo</th><th>Rango</th></tr>
				...
				if (($frm_search)!="")
					{$sql="SELECT grp_id, grp_desc, grp_rango FROM grupo WHERE grp_desc like '%$frm_search%'";}					
				else
					{$sql="SELECT grp_id, grp_desc, grp_rango FROM grupo";}
				$rs=odbc_exec($conn,$sql);
				while (odbc_fetch_row($rs))
				{
					$id=odbc_result($rs,"grp_id");
					$grupo=odbc_result($rs,"grp_desc");
